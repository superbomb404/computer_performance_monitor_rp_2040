...
本分支是添加了 ROG Logo 开机动画的版本。设备上电后会以 25fps 播放 65 帧逐帧动画一遍，最后一帧额外停留 1.6 秒，然后进入原本的性能监视主界面。

本版本额外加入上位机安装盘模式：如果设备上电后 30 秒内没有收到电脑上位机握手，会自动把 USB 设备切换为 CDC 串口 + 只读虚拟 U 盘的复合设备。U 盘中只包含标准版上位机压缩包 `computer_performance_monitor-v1.2.0.zip`。屏幕会显示 U 盘模式提示，打开压缩包运行上位机并连接设备后，设备会自动退出 U 盘模式并回到性能监视主界面。
