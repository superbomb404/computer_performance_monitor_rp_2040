...
`v1.2.1` 正常版不包含开机动画，保留性能监视、两位小数内存/显存显示、USB CDC 通信和自动上位机安装盘功能。如果设备上电后 30 秒内没有收到电脑上位机握手，会自动切换为 CDC 串口 + 只读 FAT12 虚拟 U 盘复合设备，U 盘内包含标准版上位机压缩包 `computer_performance_monitor-v1.2.0.zip`；上位机连接设备后会自动退出 U 盘模式。
