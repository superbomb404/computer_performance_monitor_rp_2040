...
本分支是添加了 Doro 开机动画的版本。设备上电后会先以 25fps 播放 Doro 逐帧动画三遍，然后进入原本的性能监视主界面。正常正式版本请继续使用 `main` 分支或 `v1.1.0` release。

本版本额外加入上位机安装盘模式：如果设备上电后 30 秒内没有收到电脑上位机握手，会自动把 USB 设备切换为 CDC 串口 + 只读虚拟 U 盘的复合设备。U 盘中只包含 Hitori 恶搞版上位机压缩包 `computer_performance_monitor-hitori-v1.2.0.zip`。屏幕会显示 U 盘模式提示，打开压缩包运行上位机并连接设备后，设备会自动退出 U 盘模式并回到性能监视主界面。

`v1.2.1` 起，虚拟 U 盘改为最小 FAT12 只读镜像，并使用 `U盘logo.png` 生成整屏位图提示，避免固件字库缺字导致中文显示为方框。
