...
{
    lcd_addr_set(0, 0, STARTUP_FRAME_WIDTH - 1, STARTUP_FRAME_HEIGHT - 1);
...
static void play_startup_animation()
{
    ...
    for (uint16_t repeat_index = 0; repeat_index < STARTUP_REPEAT_COUNT; repeat_index++) {
        for (uint16_t frame_index = 0; frame_index < STARTUP_FRAME_COUNT; frame_index++) {
            ...
            if (repeat_index == STARTUP_REPEAT_COUNT - 1 && frame_index == STARTUP_FRAME_COUNT - 1) {
